...
class ImprovedAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.rotary_emb = lambda batch_size, num_heads, seq_len, head_dim: (
            jnp.sin(jnp.arange(seq_len)[:, None] * jnp.arange(head_dim)[None, :]).reshape((1, seq_len, head_dim)).repeat(batch_size, axis=0).repeat(num_heads, axis=1),
            jnp.cos(jnp.arange(seq_len)[:, None] * jnp.arange(head_dim)[None, :]).reshape((1, seq_len, head_dim)).repeat(batch_size, axis=0).repeat(num_heads, axis=1)
        )

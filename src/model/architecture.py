...
class ImprovedAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.rotary_emb = lambda batch_size, seq_len: (
            jnp.sin(jnp.arange(seq_len)[:, None] * jnp.arange(self.head_dim // 2)[None, :]).reshape(1, 1, seq_len, self.head_dim // 2).repeat(batch_size, axis=0).repeat(self.num_heads, axis=1),
            jnp.cos(jnp.arange(seq_len)[:, None] * jnp.arange(self.head_dim // 2)[None, :]).reshape(1, 1, seq_len, self.head_dim // 2).repeat(batch_size, axis=0).repeat(self.num_heads, axis=1)
        )

...
class ImprovedAttention(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, mask: Optional[jnp.ndarray] = None, kv_cache: Optional[jnp.ndarray] = None):
        ...
        expected_embed_dim = self.num_heads * self.head_dim * 2
        if embed_dim != expected_embed_dim:
            if embed_dim == self.head_dim:
                ...
                x = x.reshape(batch_size, seq_len, embed_dim // self.head_dim, self.head_dim * 2)
                if x.shape[2] != self.num_heads:
                    raise ValueError(f"Number of heads mismatch: expected {self.num_heads}, but got {x.shape[2]}")
            if x.shape[-1] != self.head_dim * 2:
                raise ValueError(f"Head dimension mismatch: expected {self.head_dim * 2}, but got {x.shape[-1]}")
        assert x.shape == (batch_size, seq_len, self.num_heads, self.head_dim * 2), f"Embedding dimension must match num_heads * head_dim * 2, but got {x.shape} instead of {(batch_size, seq_len, self.num_heads, self.head_dim * 2)}"
...
def apply_rotary_pos_emb(x, sincos, head_dim, num_heads):
    ...
    if len(x1.shape) != 4:
        x1 = x1.reshape((x1.shape[0], x1.shape[1], 1, head_dim))

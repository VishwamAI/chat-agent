...
def apply_rotary_pos_emb(x, sincos, head_dim):
    ...
    x1 = x1.reshape(x1.shape[:-1] + (head_dim,))
    x2 = x2.reshape(x2.shape[:-1] + (head_dim,))
    sin = sin.reshape((1, sin.shape[1], sin.shape[2], head_dim)).transpose(0, 1, 3, 2)
    cos = cos.reshape((1, cos.shape[1], cos.shape[2], head_dim)).transpose(0, 1, 3, 2)

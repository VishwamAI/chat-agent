...
def train_step(params, transformed_forward, optimizer, batch, labels, rng):
    ...
    """
    def loss_fn(params):
        logits = transformed_forward.apply(params, rng, batch)  # Pass rng and batch to the model call
    ...
    batch_jax = jax.device_put(tf.cast(batch, tf.int32))
    labels_jax = jax.device_put(tf.cast(labels, tf.int32))
...
def train_model(data_file, num_epochs=10, batch_size=8):
    ...
    params = transformed_forward.init(transformer_rng, example_batch)

    # Training loop
    for epoch in range(num_epochs):
        for batch in data_generator(data_file, batch_size=batch_size, label_encoder=label_encoder):
            batch, labels = batch
            batch = np.array(batch, dtype=np.int32)  # Ensure batch is numpy array
            labels = np.array(labels, dtype=np.int32)  # Ensure labels are numpy array

...
class VishwamAIModel(hk.Module):
    ...
    def __call__(self, inputs):
        ...
        for i, expert in enumerate(self.experts):
            mask = (expert_indices[:, None] == i)  # Expand expert_indices to include batch and sequence length dimensions
            mask = jnp.broadcast_to(mask, (inputs.shape[0], embedded_inputs.shape[1]))  # Ensure mask is broadcast-compatible with batch, sequence length, and embedding dimensions

...
from vishwamai.xla_model_parallel import (
...
class Sampler(nn.Module):
    ...
    def __init__(self, vocab_size: int, world_size: int, rank: int,
                 config: vishwamai_config.VishwamaiConfig) -> None:
        super().__init__()
    ...
    def __init__(
        ...
        for i in range(config.num_hidden_layers):
            if config.architecture == vishwamai_config.Architecture.VISHWAMAI_1:
                ...
                self.layers.append(
                    Vishwamai2DecoderLayer(config, attn_type, world_size, rank))
            else:
...
class VishwamaiForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.model = VishwamaiModel(config, world_size, rank)

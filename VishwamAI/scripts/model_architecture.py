...
class VishwamAIModel(hk.Module):
    ...
    def __call__(self, inputs):
        ...
        for i, expert in enumerate(self.experts):
            mask = (expert_indices[:, None, None, None] == i)  # Expand expert_indices to include batch, sequence length, and embedding dimensions
            mask = jnp.broadcast_to(mask, (inputs.shape[0], embedded_inputs.shape[1], embedded_inputs.shape[2], embedded_inputs.shape[3]))  # Ensure mask is broadcast-compatible with batch, sequence length, and embedding dimensions
            print(f"Shape of mask before expand_dims: {mask.shape}")
            print(f"Shape of mask after expand_dims: {mask.shape}")

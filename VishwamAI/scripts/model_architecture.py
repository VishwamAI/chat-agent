...
class VishwamAIModel(hk.Module):
    ...
    def __call__(self, inputs):
        ...
        for i, expert in enumerate(self.experts):
            ...
            mask = jnp.broadcast_to(mask[:, :, None], (inputs.shape[0], embedded_inputs.shape[1], 1, mask.shape[-1]))  # Ensure mask is broadcast-compatible with batch and sequence length dimensions while preserving the last dimension

...
class ImprovedAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.rotary_emb = lambda batch_size, num_heads, seq_len: (
            jnp.sin(jnp.arange(seq_len)[:, None] * jnp.arange(self.head_dim)[None, :]).reshape(seq_len, self.head_dim).reshape(1, num_heads, seq_len, self.head_dim).repeat(batch_size, axis=0),
            jnp.cos(jnp.arange(seq_len)[:, None] * jnp.arange(self.head_dim)[None, :]).reshape(seq_len, self.head_dim).reshape(1, num_heads, seq_len, self.head_dim).repeat(batch_size, axis=0)
        )

...
class RotaryEmbedding(hk.Module):
    ...
    def __call__(self, seq_len):
        ...
        sin = jnp.sin(freqs).reshape(1, seq_len, self.head_dim // 2, 2).transpose(0, 1, 3, 2).reshape(1, seq_len, self.num_heads, self.head_dim)
        cos = jnp.cos(freqs).reshape(1, seq_len, self.head_dim // 2, 2).transpose(0, 1, 3, 2).reshape(1, seq_len, self.num_heads, self.head_dim)

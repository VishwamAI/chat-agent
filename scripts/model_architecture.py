    ...
    def __init__(self, transformer_model_name="gpt2"):
        ...
        self.transformer = hk.transform(
            lambda x: hk.Sequential([
                hk.Embed(vocab_size=50257, embed_dim=512, w_init=hk.initializers.VarianceScaling(1.0, "fan_avg", "uniform"))(jax.numpy.array(x, dtype=jnp.int32)),
        ...
        class CustomAttentionLayer(hk.Module):
            def __init__(self, tokenizer, transformer, memory_network):
                ...
                self.transformer_xl = hk.transform(
                    lambda x: hk.Sequential([
                        lambda x: hk.Embed(vocab_size=50257, embed_dim=512, w_init=hk.initializers.VarianceScaling(1.0, "fan_avg", "uniform"))(jax.numpy.array(x, dtype=jnp.int32)),
                        lambda x: x,
